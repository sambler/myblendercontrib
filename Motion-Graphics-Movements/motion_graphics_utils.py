...
def setLatticeAsBoundingBox(lattice, target):
	location = getBoundingBoxCenter(target)[:]
	lattice.location = location
	lattice.rotation_euler = target.rotation_euler
	lattice.scale = target.dimensions
	
def getBoundingBoxCenter(object):
	deselectAll()
	setActive(object)
	oldCursorPosition = bpy.context.scene.cursor_location[:]
	oldOrigin = object.location
	bpy.context.scene.cursor_location = oldOrigin
	bpy.ops.object.origin_set(type = 'ORIGIN_GEOMETRY', center = 'BOUNDS')
	newOrigin = object.location[:]
	bpy.ops.object.origin_set(type = "ORIGIN_CURSOR")
	bpy.context.scene.cursor_location = oldCursorPosition
	return newOrigin
