...
def sort_parent_and_child(obs, ob_children={}):
    '''親から順に返す'''
    if not ob_children:
        obs_set = set(obs)  # 長大なシーケンスの場合に効率が上がる
        ob_parent = {ob:None for ob in obs}
        for ob in obs:
            if ob.parent in obs_set:
                ob_parent[ob] = ob.parent
        ob_children = {ob:[] for ob in obs}
        for ob, parent in ob_parent.items():
            if parent:
                ob_children[parent].append(ob)
        no_parent = [ob for ob, parent in ob_parent.items() if not parent]
        obs = no_parent

    for ob in obs:
        yield ob
        for o in sort_parent_and_child(ob_children[ob], ob_children):
            yield o


### Operator ##################################################################
def selobs_align_to_rotmat(context, mat, except_actob=False):
    '''
    m1 * m2 = m3
    m1 = m3 * m2.invert()
    m2 = m1.invert() * m3
    ...
    # reset rotation
    '''
    ob.rotation_euler = [0, 0, 0]では、
    ob.matrix_world.rotation_part()が更新されない
    '''
    actobselect = False
    if except_actob:
    ...
    for ob in selobs:
        if ob.parent:
            obrotmat = ob.matrix_world.rotation_part()
            rotmat = obrotmat.copy().invert() * mat
        else:
            rotmat = mat
