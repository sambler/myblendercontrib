...
class AddTree(bpy.types.Operator):
    ...
    leafScaleX = FloatProperty(name='Leaf Scale X',
        description='The scaling applied to the x direction of the leaf (LeafScaleX)',
        min = 0.0,
        default=1.0)
    ...
    frameRate = FloatProperty(name='Frame Rate',
        description='The number of frames per second which can be used to adjust the speed of animation',
        min=0.001,
        default=1)
    ...
    limitImport = BoolProperty(name='Limit Import',
        description='Limited the imported tree to 2 levels and no leaves for speed',
        default=True)
...
#        if self.chooseSet == '0':
            ...
            row.operator('sapling.export').data = repr([repr(data),self.presetName])
            row = box.row()
