...
def vgroups_draw(self, context):
    ...
    layout.row().operator("object.add_corrective_pose_shape_fast", text='Add as corrective pose-shape (faster)', icon='COPY_ID') # icon is not ideal
    layout.row().operator("object.add_corrective_pose_shape", text='Add as corrective pose-shape (slower)', icon='COPY_ID') # icon is not ideal
...
def unregister():
    ...
    bpy.types.MESH_MT_shape_key_specials.remove(vgroups_draw)
