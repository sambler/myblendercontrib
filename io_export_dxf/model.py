...
class MigusDXFLibDrawing(DxfDrawing):
	""" Drawing that can convert itself into MIGUS DXFLib stuff objects """
	def __init__(self, **kwargs):
		DxfDrawing.__init__(self)
		...
		try:
			...
			raise Exception("DXF-Exporter: error! found no dxfLibrary.py module in Blender script folder")
	...
	def convert(self, file, **kwargs):
		...
		for v in self._views:
			drawing.views.append(self.DXF.View(**v))
		for v in self._vports:
			drawing.vports.append(self.DXF.VPort(**v))
#		for l in self._layers:
	...
	def _write(self):		
		...
		d.linetypes.append(self.DXF.LineType(name='DOT',description='. . . . . . .',elements=[0.25, 0.0, -0.25]))
		d.linetypes.append(self.DXF.LineType(name='DASHED',description='__ __ __ __ __',elements=[0.8, 0.5, -0.3]))
		d.linetypes.append(self.DXF.LineType(name='DASHDOT',description='__ . __ . __ .',elements=[1.0, 0.5, -0.25, 0.0, -0.25]))
		d.linetypes.append(self.DXF.LineType(name='DIVIDE',description='____ . . ____ . . ',elements=[1.25, 0.5, -0.25, 0.0, -0.25, 0.0, -0.25]))
		d.linetypes.append(self.DXF.LineType(name='BORDER',description='__ __ . __ __ . ',elements=[1.75, 0.5, -0.25, 0.5, -0.25, 0.0, -0.25]))
		d.linetypes.append(self.DXF.LineType(name='HIDDEN',description='__ __ __ __ __',elements=[0.4, 0.25, -0.25]))
		d.linetypes.append(self.DXF.LineType(name='CENTER',description='____ _ ____ _ __',elements=[2.0, 1.25, -0.25, 0.25, -0.25]))
	
		#d.vports.append(self.DXF.VPort('*ACTIVE'))
		d.vports.append(self.DXF.VPort('*ACTIVE',center=(-5.0,1.0),height=10.0))
		#d.vports.append(self.DXF.VPort('*ACTIVE',leftBottom=(-100.0,-60.0),rightTop=(100.0,60.0)))
		#d.views.append(self.DXF.View('Normal'))	  #table view
		d.views.append(self.DXF.ViewByWindow('BF_TOPVIEW',leftBottom=(-100,-60),rightTop=(100,60)))  #idem
	...
	def _processEntityArray(self, drawing, type, ents):
		if type=='Point':
			for e in ents:
				drawing.append(self.DXF.Point(**e))
		elif type=='Line':
			for e in ents:
				drawing.append(self.DXF.Line(**e))
		elif type=='PolyLine':
			for e in ents:
				drawing.append(self.DXF.PolyLine(**e))
		elif type=='Face':
			for e in ents:
				drawing.append(self.DXF.Face(**e))
