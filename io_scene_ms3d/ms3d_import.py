...
if ('bpy' in locals()):
    ...
    from io_scene_ms3d.ms3d_ui import (
            ms3d_flags_to_ui,
            ms3d_transparency_mode_to_ui,
            ms3d_texture_mode_to_ui,
            )
    pass
...
class Ms3dImporter():
    ...
    def read(self, blender_context):
        ...
        t3 = time()
        print(ms3d_str['SUMMARY_IMPORT'].format((t3 - t1), (t2 - t1), (t3 - t2)))
    ...
    def create_geometry(self, blender_context, ms3d_model):
        ...
        if ms3d_model_ex is not None:
            ...
            blender_mesh.ms3d.transparency_mode = ms3d_transparency_mode_to_ui(ms3d_model_ex.transparency_mode)

        ##########################
        # blender stuff:
        # link to blender object
        blender_mesh_object = bpy.data.objects.new(ms3d_model.name + ".m", blender_mesh)

        ##########################
        # blender stuff:
        # create edge split modifire, to make sharp edges visible
        blender_modifier = blender_mesh_object.modifiers.new("ms3d_smoothing_groups", type='EDGE_SPLIT')
        ...
        for ms3d_group_index, ms3d_group in enumerate(ms3d_model.groups):
            ...
            blender_group.flags = ms3d_flags_to_ui(ms3d_group.flags)
        ...
        for ms3d_vertex_index, ms3d_vertex in enumerate(ms3d_model.vertices):
            bmv = bm.verts.new(self.matrix_scaled_coordination_system * Vector(ms3d_vertex.vertex))

        ##########################
        # blender stuff (uses BMesh stuff):
        # create all materials / image textures
        ms3d_to_blender_material = {}
        for ms3d_material_index, ms3d_material in enumerate(ms3d_model.materials):
            ...
            blender_material.ms3d.mode = ms3d_texture_mode_to_ui(ms3d_material.mode)
            ...
            if ms3d_material.texture:
                ...
                blender_image_diffuse = load_image(file_name_diffuse, dir_name_diffuse)
                blender_texture_diffuse = bpy.data.textures.new(name=file_name_diffuse, type='IMAGE')
                blender_texture_diffuse.image = blender_image_diffuse
                blender_texture_slot_diffuse = blender_material.texture_slots.add()
            ...
            if ms3d_material.alphamap:
                ...
                blender_image_alpha = load_image(file_name_alpha, dir_name_alpha)
                blender_texture_alpha = bpy.data.textures.new(name=file_name_alpha, type='IMAGE')
                blender_texture_alpha.image = blender_image_alpha
                blender_texture_slot_alpha = blender_material.texture_slots.add()
            ...
            # translation dictionary
            ms3d_to_blender_material[ms3d_material_index] = blender_image_diffuse

        ##########################
        # BMesh stuff:
        # create all triangles
        smoothing_group_blender_faces = {}
        for ms3d_triangle_index, ms3d_triangle in enumerate(ms3d_model.triangles):
            bmv_list = []
            for vert_index in ms3d_triangle.vertex_indices:
                bmv = bm.verts[vert_index]
                if [[x] for x in bmv_list if x == bmv]:
                    self.options.report({'WARNING', 'INFO'}, ms3d_str['WARNING_IMPORT_SKIP_VERTEX_DOUBLE'].format(ms3d_triangle_index))
                    continue
                bmv_list.append(bmv)

            if len(bmv_list) < 3:
                self.options.report({'WARNING', 'INFO'}, ms3d_str['WARNING_IMPORT_SKIP_LESS_VERTICES'].format(ms3d_triangle_index))
                continue
            bmf = bm.faces.get(bmv_list)
            if bmf is not None:
                self.options.report({'WARNING', 'INFO'}, ms3d_str['WARNING_IMPORT_SKIP_FACE_DOUBLE'].format(ms3d_triangle_index))
                continue

            bmf = bm.faces.new(bmv_list)

            # blender uv custom data per "face vertex"
            bmf.loops[0][layer_uv].uv = Vector((ms3d_triangle.s[0], 1.0 - ms3d_triangle.t[0]))
            bmf.loops[1][layer_uv].uv = Vector((ms3d_triangle.s[1], 1.0 - ms3d_triangle.t[1]))
            bmf.loops[2][layer_uv].uv = Vector((ms3d_triangle.s[2], 1.0 - ms3d_triangle.t[2]))

            # ms3d custom data per "mesh face"
            bmf[layer_smoothing_group] = ms3d_triangle.smoothing_group

            blender_group_id = ms3d_to_blender_group_index.get(ms3d_triangle.group_index)
            if blender_group_id is not None:
                bmf[layer_group] = blender_group_id

            ms3d_material_index = ms3d_model.groups[ms3d_triangle.group_index].material_index
            if ms3d_material_index != NONE_GROUP_MATERIAL_INDEX:
                ...
                bmf[layer_texture].image = ms3d_to_blender_material.get(ms3d_material_index)
            else:
                # set material index to highes possible index - in most cases there is no maretial assigned ;)
                bmf.material_index = 32766

            # helper dictionary for post-processing smoothing_groups
            if smoothing_group_blender_faces.get(ms3d_triangle.smoothing_group) is None:
        ...
        for ms3d_smoothing_group_index, blender_face_list in smoothing_group_blender_faces.items():
            edge_dict = {}
    ...
    def create_animation(self, blender_context, ms3d_model, blender_mesh_object):
        ...
        blender_armature_object = bpy.data.objects.new(ms3d_armature_name, blender_armature)
        ...
        for ms3d_vertex_index, ms3d_vertex in enumerate(ms3d_model.vertices):
            ...
            if ms3d_vertex.bone_id != NONE_VERTEX_BONE_ID:
                blender_vertex_group = ms3d_to_blender_vertex_groups.get(ms3d_vertex.bone_id)
                if blender_vertex_group is None:
                    ms3d_to_blender_vertex_groups[ms3d_vertex.bone_id] = blender_vertex_group = []
                blender_vertex_group.append(ms3d_vertex_index)

        ##########################
        # blender stuff:
        # create all vertex groups to be used for bones
        for ms3d_bone_id, blender_vertex_index_list in ms3d_to_blender_vertex_groups.items():
            ms3d_name = ms3d_model.joints[ms3d_bone_id].name
            blender_vertex_group = blender_mesh_object.vertex_groups.new(ms3d_name)
            blender_vertex_group.add(blender_vertex_index_list, 1.0, 'REPLACE')

        blender_modifier = blender_mesh_object.modifiers.new(ms3d_armature_name, type='ARMATURE')
        blender_modifier.show_expanded = False
        ...
        for ms3d_joint in ms3d_model.joints:
            ...
            matrix_local = (Matrix.Rotation(ms3d_joint.rotation[2], 4, 'Z') * Matrix.Rotation(ms3d_joint.rotation[1], 4, 'Y')) * Matrix.Rotation(ms3d_joint.rotation[0], 4, 'X')
            matrix_local = Matrix.Translation(Vector(ms3d_joint.position)) * matrix_local

            ms3d_joint.matrix_local = matrix_local
            ms3d_joint.matrix_global = matrix_global = matrix_local

            if ms3d_joint.parent_name:
                ms3d_joint_parent = ms3d_joint_by_name.get(ms3d_joint.parent_name)
                if ms3d_joint_parent is not None:
                    ms3d_joint_parent.children.append(ms3d_joint)

                    matrix_global = ms3d_joint_parent.matrix_global * matrix_global
        ...
        for ms3d_joint in ms3d_model.joints:
            ...
            if True:
                blender_edit_bone.head = self.matrix_scaled_coordination_system * ms3d_joint_vector

                number_children = len(ms3d_joint.children)
                if number_children > 0:
                    vector_midpoint = Vector()
                    for item_child in ms3d_joint.children:
                        ms3d_joint_child_vector = ms3d_joint_by_name[item_child.name].matrix_global * Vector()
                        vector_midpoint += ms3d_joint_child_vector - ms3d_joint_vector
                    vector_midpoint /= number_children
                    blender_edit_bone.tail = blender_edit_bone.head + self.matrix_scaled_coordination_system * vector_midpoint
                else:
                    vector_tail_end = Vector()
                    if ms3d_joint.parent_name:
                        ms3d_joint_parent_vector = ms3d_joint_by_name[ms3d_joint.parent_name].matrix_global * Vector()
                        vector_tail_end = ms3d_joint_vector - ms3d_joint_parent_vector
                    else:
                        #dummy tail
                        vector_tail_end = ms3d_joint.matrix_global * Vector()
                    vector_tail_end.normalize()
                    blender_edit_bone.tail = blender_edit_bone.head + self.matrix_scaled_coordination_system * vector_tail_end
            else:
                blender_edit_bone.head = self.matrix_scaled_coordination_system * ms3d_joint_vector

                if ms3d_joint.parent_name:
                    vector_parent = ms3d_joint_by_name[ms3d_joint.parent_name].matrix_global * Vector()
                    blender_edit_bone.tail = self.matrix_scaled_coordination_system * vector_parent
                else:
                    number_children = len(ms3d_joint.children)
                    if number_children > 0:
                        vector_midpoint = Vector()
        ...
        for ms3d_joint_name, ms3d_joint in ms3d_joint_by_name.items():
            blender_bone = blender_armature.bones.get(ms3d_joint.blender_bone_name)
            if blender_bone is None:
                continue

            blender_bone.ms3d.name = ms3d_joint.name
            blender_bone.ms3d.flags = ms3d_flags_to_ui(ms3d_joint.flags)
        ...
        for ms3d_joint_name, ms3d_joint  in ms3d_joint_by_name.items():
            blender_pose_bone = blender_armature_object.pose.bones.get(ms3d_joint.blender_bone_name)
            if blender_pose_bone is None:
                continue
            ms3d_joint_local_matrix = ms3d_joint.matrix_local ##ms3d_joint_by_name[ms3d_joint.name][get_ms3d_joint_local_matrix]
            ms3d_joint_global_matrix = ms3d_joint.matrix_global ##ms3d_joint_by_name[ms3d_joint.name][get_ms3d_joint_global_matrix]
            ...
            for translation_key_frames in ms3d_joint.translation_key_frames:
                ...
                matrix_local = Matrix.Translation(Vector(translation_key_frames.position))
                v = self.matrix_scaled_coordination_system * (matrix_local * Vector())
            ...
            for rotation_key_frames in ms3d_joint.rotation_key_frames:
                ...
                matrix_local = (Matrix.Rotation(rotation_key_frames.rotation[2], 4, 'Z') * Matrix.Rotation(rotation_key_frames.rotation[1], 4, 'Y')) * Matrix.Rotation(rotation_key_frames.rotation[0], 4, 'X')
