...
class MESH_OT_vertex_slide(bpy.types.Operator):
    ...
    def vertex_slide(self, context):
        ...
        edgelines[:] = []  # Clear
        translines[:] = []  # Clear
        for vert in (v for v in pymesh.vertices if v.is_selected and v.vertices):
            connected_vertices = [v for v in vert.vertices if not v.hide]
            ...
            for i in range(len(connected_vertices)):
                ...
                if (v.worco - vert.worco).length < MIN_NUMBER:
                    connected_vertices.extend([vv for vv in v.vertices if not vv.hide])
            angles = []
            zerolength = True
    ...
    def reset_vertices(self, context=None):
        ...
        for vert in me.vertices:
            vert.co = pymesh.vertices[vert.index].co
        bpy.ops.object.mode_set(mode='EDIT')
